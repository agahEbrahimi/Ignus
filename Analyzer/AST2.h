...
// ControlStatements: NULL condition and process type
...
class Class : public BaseStatement {
   ...
   virtual CompileStatement verify(vector<DictLevel>& dictionary)
   {
    ...
    //TODO: CHECK IF I WAS SUPPOSED TO DO THIS
    ...
    DictLevel layer = DictLevel(); //TODO
    dictionary.push_back(layer);
      ...
      for(vector<DictLevel>::size_type i = 0; i != block.statements.size(); i++) 
      {
        ...
        //dictentry.nodemap[entry.name] = entry;
        dictentry.nodemap->insert(std::pair<string, DictEntry>(entry.name, entry));
        //cout<< "added: " << entry.name << " !!!!!!!\n";
        //cout<< "maybe: " << dictentry.nodemap[entry.name].typeName << "\n";
        //entry.name = id.name.append(".").append(entry.name);
        //cout << entry.name << "\n";
        //DictLevel layer = dictionary.back();
        //dictionary.pop_back();
        
      }
   
    
      dictionary.pop_back(); //TODO
...
class BinaryOperator : public Expression {
   ...
   virtual CompileStatement verifyOld(vector<DictLevel>& dict)
   {
      cout<<"Verifying BinaryOperator - WARN OLD METHOD\n";
    //  cout<<"\t";
    //   for(int i = 0; i < dict.size(); i++)
    //     cout<<"\t";
    //   cout<< lhs.debugString() << " " << binOperName(oper) << " " << rhs.debugString() << "\n";
      string typerhs = "";
      string typelhs = "";
...
class ControlStatements : public Control {
   ...
   virtual CompileStatement verify(vector<DictLevel>& dict) //TODO: Null condition? AND CHECK TYPE
   {
...
//For measuring elapsed time: https://stackoverflow.com/questions/2808398/easily-measure-elapsed-time
   ...
   for(vector<DictLevel>::size_type i = 0; i != table.size(); i++) 
   {
      ...
      while(it != table[i].end())
      {
         ...
         //unordered_map<string, DictEntry>::iterator nests = it->
           
           /* for(const auto curr : it->second.getDisplay())
            {
                cout<< curr << "\n";
            }
            */
